...
- Use the <a href="../../Owl Diet.csv" download>Owl Diet.csv</a> dataset
...
1. Load the CSV data with `readr::read_csv()`
2. Fit the discriminant model with `MASS::lda()`
3. Review group separation and classifications
...
install.packages(c("MASS", "readr"), repos = "https://cloud.r-project.org")
library(readr)
library(MASS)

# Read the data file
data <- readr::read_csv("../../Owl Diet.csv")
str(data)
...
1. Load the CSV data with `readr::read_csv()`
2. Fit the MANOVA with `manova()`
3. Review multivariate test statistics with `summary()`
...
# Load the dataset
data_1 <- readr::read_csv("../../Blood Pressure.csv")
...
1. Load the CSV data with `read.csv()`
2. Fit the PCA with `prcomp()`
3. Review variance explained and the scree plot
...
1. Load the `.sav` file with `haven::read_sav()`
2. Fit the factor model with `psych::fa()`
3. Review factor loadings, variance explained, and the factor diagram
...
install.packages(c("haven", "psych"), repos = "https://cloud.r-project.org")
library(haven)
library(psych)
