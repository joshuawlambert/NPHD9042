...
- Read `hsb2.csv` and create a factor outcome indicating whether `science` is above the sample mean.
- Use `set.seed()` before randomly splitting the rows into training, validation, and test sets.
- Use `rpart::rpart(..., method = "class")` to fit the classification tree.
- Exclude `id`, the original `science` score, and the new outcome from the predictor set where appropriate.
- Use `rpart.plot::rpart.plot()` to create the tree figure.
- Use `predict(..., type = "class")` and compare predicted versus observed classes to calculate misclassification rates.
