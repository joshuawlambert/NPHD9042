...
- Read `hsb2.csv` and keep variable names lowercase (`read`, `write`, `math`, `science`, `socst`, `female`).
- Use `lavaan::sem()` for both the observed-variable path model and the SEM model.
- In lavaan syntax, use `~` for regressions and `=~` for latent-variable measurement.
- Label parameters as shown in the diagrams so estimates map back to the figures.
