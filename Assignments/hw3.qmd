...
# Question 1: observed-variable path model from the first diagram
path_model <- '
  math ~ a1*read + a2*write
  science ~ b1*read + b2*math + b3*write
'

path_fit <- lavaan::sem(path_model, data = hsb2)
summary(path_fit, fit.measures = TRUE, standardized = TRUE, rsquare = TRUE)
parameterEstimates(path_fit, standardized = TRUE)

# Question 2: SEM model from the second diagram
sem_model <- '
  Acad =~ 1*math + a3*science + a4*socst
  read ~ a1*female
  Acad ~ a2*read
'

sem_fit <- lavaan::sem(sem_model, data = hsb2)
