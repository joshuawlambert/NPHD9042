...
- Read `HW2.csv`, then convert categorical predictors (`race`, `edu`, `smoke`, `diab`) to factors.
- Build single-predictor `lm()` models first, then a full multivariable `lm()` model.
- Use `MASS::stepAIC()` for forward/backward selection if using AIC.
- Use nested `lm()` models and `anova()` for the hierarchical regression section.
- Use `glm(..., family = binomial)` for the logistic outcome and exponentiate coefficients/confidence intervals for odds ratios.
- Keep a clean table of estimates, p-values, and confidence intervals for each final model.
