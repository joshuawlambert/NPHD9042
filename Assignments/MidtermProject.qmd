...
- Download the chosen raw dataset as a CSV and read it with `read.csv()` or `readr::read_csv()`.
- Use the dataset codebook to identify the item columns that belong in the EFA; remove IDs, timestamps, demographics, or scoring keys that are not items.
- Check missingness and response coding before computing correlations.
- Use `psych::fa()` for extraction and rotation, and justify the number of factors using eigenvalues, a scree plot, parallel analysis, and theory.
- Save tables/figures from R that match the Methods and Results language in the paper.
